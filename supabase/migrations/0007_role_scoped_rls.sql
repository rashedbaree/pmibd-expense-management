...
-- policies from 0001_init.sql (and 0006's bank_statement_lines policy) with
-- role-scoped policies that match the application's actual access model.
-- Run in the SQL Editor after 0006_bank_statement.sql.
--
-- The read policies on expenses/expense_documents/expense_approvals set up
-- here are further tightened by 0008_full_visibility.sql to respect
-- per-portfolio/per-role visibility instead of being fully org-wide.
--
-- Access model:
