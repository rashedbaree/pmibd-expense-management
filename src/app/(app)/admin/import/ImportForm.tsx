...
import { importExpenses, type ImportResult } from "./actions";

const initialState: ImportResult = { successCount: 0, errors: [] };

export default function ImportForm() {
  const [state, formAction, isPending] = useActionState(importExpenses, initialState);
    ...
    <div className="mt-4 flex flex-col gap-4">
      ...
      {state.successCount > 0 && (
        <p className="rounded-md bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400">
          Imported {state.successCount} expense{state.successCount === 1 ? "" : "s"} as Paid.
        </p>
      )}
