import type { createClient } from "./supabase/server";
import { scopeExpenseQuery, type VisibilityScope } from "./visibility";
import { signedAmount } from "./expense";
import type { EntryType, ExpenseStatus } from "./types";

type SupabaseClient = Awaited<ReturnType<typeof createClient>>;

type Row = {
  id: string;
  date: string;
  amount: number;
  status: ExpenseStatus;
  entry_type: EntryType;
  portfolio: { name: string } | null;
  category: { name: string } | null;
  event: { name: string } | null;
};

export type ReportRow = { label: string; count: number; amount: number };

export type DateRangeFilters = { date_from?: string; date_to?: string; all?: string };

export type DateRange = { dateFrom: string; dateTo: string; showAll: boolean };

function isoDate(d: Date) {
  return d.toISOString().slice(0, 10);
}

// Defaults to the last 12 months so a page built from this stays readable -
// "All time" (or an explicit date range) opts out of that default.
export function resolveDateRange(filters: DateRangeFilters): DateRange {
  const showAll = filters.all === "1";
  const today = new Date();
  const defaultFromDate = new Date(today);
  defaultFromDate.setMonth(defaultFromDate.getMonth() - 11, 1);
  const defaultFrom = isoDate(defaultFromDate);
  const defaultTo = isoDate(today);

  const dateFrom = showAll ? (filters.date_from ?? "") : (filters.date_from || defaultFrom);
  const dateTo = showAll ? (filters.date_to ?? "") : (filters.date_to || defaultTo);

  return { dateFrom, dateTo, showAll };
}

export type SpendBreakdown = {
  periodWise: ReportRow[];
  categoryWise: ReportRow[];
  portfolioWise: ReportRow[];
  eventWise: ReportRow[];
  approvalStatus: ReportRow[];
};

export async function getSpendBreakdown(
  supabase: SupabaseClient,
  scope: VisibilityScope,
  range: DateRange,
): Promise<SpendBreakdown> {
  let query = supabase
    .from("expenses")
    .select(
      `id, date, amount, status, entry_type,
       portfolio:portfolios(name),
       category:expense_categories(name),
       event:events(name)`,
    )
    .order("date", { ascending: false });
  query = scopeExpenseQuery(query, scope);
  if (range.dateFrom) query = query.gte("date", range.dateFrom);
  if (range.dateTo) query = query.lte("date", range.dateTo);

  const { data: raw } = await query;
  const expenses = (raw ?? []) as unknown as Row[];

  // Approved + paid only - draft/pending/returned/rejected expenses were
  // never actually spent, so counting them here would overstate spend.
  // Approval Status is the one place every status belongs, since its whole
  // point is showing the distribution across them.
  const spendExpenses = expenses.filter(
    (e) => e.status === "approved" || e.status === "paid",
  );

  const sumBy = (rows: Row[], key: (e: Row) => string) => {
    const map = new Map<string, { count: number; amount: number }>();
    for (const e of rows) {
      const k = key(e);
      const entry = map.get(k) ?? { count: 0, amount: 0 };
      entry.count += 1;
      entry.amount += signedAmount(e);
      map.set(k, entry);
    }
    return [...map.entries()]
      .map(([label, v]) => ({ label, ...v }))
      .sort((a, b) => b.amount - a.amount);
  };

  const periodWise = sumBy(spendExpenses, (e) => e.date.slice(0, 7)).sort((a, b) =>
    b.label.localeCompare(a.label),
  );
  const categoryWise = sumBy(spendExpenses, (e) => e.category?.name ?? "Uncategorized");
  const portfolioWise = sumBy(spendExpenses, (e) => e.portfolio?.name ?? "Unassigned");
  const eventWise = sumBy(spendExpenses, (e) => e.event?.name ?? "No event");
  const approvalStatus = sumBy(expenses, (e) => e.status);

  return { periodWise, categoryWise, portfolioWise, eventWise, approvalStatus };
}
